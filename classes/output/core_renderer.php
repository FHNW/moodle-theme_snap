...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function login_button() {
        ...
        $output = '';
        $loginurl = $CFG->wwwroot.'/auth/shibboleth';
        $loginatts = [
            'aria-haspopup' => 'true',
    ...
    public function personal_menu() {
        ...
        if ($_SERVER['HTTP_HOST'] !== 'moodle.assessment.fhnw.ch') {
            $eventoimportlink = [
                'link' => s($CFG->wwwroot). '/local/evento/modulanlaesse.php',
                'title' => 'Kursraumbestellung'
            ];
            $quicklinks = [$profilelink, $dashboardlink, $preferenceslink, $gradelink, $eventoimportlink, $logoutlink];
        } else {
    ...
    public function site_frontpage_news() {
        ...
        foreach ($discussions as $discussion) {
            ...
            $readmorebtn = "<a class='btn btn-primary toggle' href='".
    ...
    public function feature_spot_cards() {
        ...
        if ($fscount > 0) {
            ...
            if (!empty($PAGE->theme->settings->fs_heading)) {
                $fstitle = '<h2 class="snap-feature-spots-heading">' .format_text($PAGE->theme->settings->fs_heading). '</h2>';
            }
    ...
    protected function feature_spot_card($title, $image, $text) {
        $card = '<div class="snap-feature">
            ...
            <div class="snap-feature-block">
                ' .$image. '
                <!--Title-->
                <h3 class="snap-feature-title h5">' .format_text($title). '</h3>
                <!--Content-->
...
HTML;
    ...
    public function activity_navigation() {
        ...
        // FHNW Customization for rendering the activity navigation in quiz or not
        if ($CFG->show_activity_navigation_in_quiz != true && $this->page->cm->modname == 'quiz') {
            return '';
        }
